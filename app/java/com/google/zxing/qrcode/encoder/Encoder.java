...
public final class Encoder {
  ...
  public static QRCode encodecodeword(byte[][] codewords,
    ...
    //codewordを各RSブロックの情報点と検査点で分けて、インターリーブ配置に従ってfainalBitsに格納
    //このファイル上のinterleaveWithECBytesを参照して作成
    BitArray finalBits = new BitArray();
    int blocknum = codewords.length;
    Collection<BlockPair> blocks = new ArrayList<>(blocknum);
    ...
    for(int i = 0; i < blocknum; i++){
      ...
      getNumDataBytesAndNumECBytesForBlockID(
          version.getTotalCodewords(), numDataBytes, blocknum, i,
          numDataBytesInBlock, numEcBytesInBlock);
          
      byte[] datacode = new byte[numDataBytesInBlock[0]];
      byte[] eccode = new byte[numEcBytesInBlock[0]];
      for(int j = 0; j < codewords[i].length; j++){
        if (j < datacode.length){
          datacode[j] = codewords[i][j];
        }else{
          eccode[j - datacode.length] = codewords[i][j];
        }  
      }
      blocks.add(new BlockPair(datacode, eccode)); 

      maxNumDataBytes = Math.max(maxNumDataBytes, numDataBytesInBlock[0]);
      maxNumEcBytes = Math.max(maxNumEcBytes, numEcBytesInBlock[0]);
    }
  ...
  // URLをQRコードの符号化過程で符号化し、1シンボル1byteの符号語として出力(ほとんどencodeのコピペ)
  // 一つの符号語しかない場合に利用可能
  public static byte[] encodebytearray(String content,
    ...
    byte[] encodebytearray = new byte[version.getTotalCodewords()];
    finalBits.toBytes(0, encodebytearray, 0, version.getTotalCodewords());
    
    return encodebytearray;
  }

  // URLをQRコードの符号化過程で符号化し、1シンボル1byteのデータコード語として出力(ほとんどencodeのコピペ)
  // URLが一つの符号語に収まっている場合のみ利用可能
  public static byte[] encodedatacodeword(String content,
                              ErrorCorrectionLevel ecLevel,
                              Map<EncodeHintType,?> hints) throws WriterException {
    ...
    byte[] encodedatacodeword = new byte[headerAndDataBits.getSizeInBytes()];
    headerAndDataBits.toBytes(0, encodedatacodeword, 0, encodedatacodeword.length);
    
    return encodedatacodeword;
  }
